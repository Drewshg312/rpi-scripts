...
# Show Battery Charge for Laptops
...
prompt_ruby() {
  ...
  if which rvm-prompt &> /dev/null; then
      ruby='%{$fg[green]%}‹ruby:$(rvm-prompt i v g)›%{$reset_color%}'
    else
    if which rbenv &> /dev/null; then
      ruby='$FG[241]‹ruby:%{$reset_color%}%{$fg[green]%}$(rbenv version-name)›%{$reset_color%}'
    fi
  fi
local return_code='%(?..%{$fg[yellow]%}%? ↵%{$reset_color%})'
prompt_segment default default "${return_code} ${ruby}"
}
